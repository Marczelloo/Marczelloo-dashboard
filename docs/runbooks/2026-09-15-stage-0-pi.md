...
Uwaga: `--env-file` Dockera nie usuwa apostrofów z wartości. Skrypt używa tylko
`ATLASHUB_API_URL` i `ATLASHUB_SECRET_KEY` — jeśli są w `.env` w cudzysłowach,
uruchom go przez `docker exec marczelloo-dashboard …` z env kontenera.
